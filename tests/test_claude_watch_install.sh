...
assert_contains "$CW_SH" "lorecraft-io/creativity-maxxing/main/claude-watch-skill" \
  "BASE_URL points at lorecraft-io/creativity-maxxing main claude-watch-skill"
...
for f in "SKILL.md" \
         "scripts/__init__.py" "scripts/channel.py" "scripts/download.py" \
...
# Hook script chmod'd executable after install.
assert_contains "$CW_SH" 're:chmod \+x .*session_start\.sh' \
  "session_start.sh chmod'd executable after install"

# Local fallback path is wired (handles offline / pre-publish state).
assert_contains "$CW_SH" 're:LOCAL_SRC=' \
  "LOCAL_SRC computed from script dir (offline-safe local fallback)"
assert_contains "$CW_SH" 're:cp .*LOCAL_SRC' \
...
# all 15 files land + self-test passes + hook is executable.
# ---------------------------------------------------------------------------
TMPROOT="$(mktemp -d -t cm-cw-XXXXXX)"
cleanup() {
...
# 11 .py files in scripts/.
PY_COUNT="$(find "$SKILL_DIR/scripts" -type f -name '*.py' 2>/dev/null | wc -l | tr -d ' ')"
assert_eq "$PY_COUNT" "11" "all 11 scripts/*.py files landed via local fallback"

# channel.py specifically — that's the playlist/channel feature.
if [[ -f "$SKILL_DIR/scripts/channel.py" ]]; then
  _pass "scripts/channel.py landed (playlist/channel mode)"
else
  _fail "scripts/channel.py missing — playlist/channel mode unavailable"
fi

# session_start hook executable.
if [[ -x "$SKILL_DIR/hooks/scripts/session_start.sh" ]]; then
  _pass "hooks/scripts/session_start.sh is executable"
else
  _fail "hooks/scripts/session_start.sh not executable"
fi
...
if [[ -f "$SKILL_DIR/hooks/hooks.json" ]]; then
  ...
  _fail "hooks/hooks.json missing"
fi

# Self-test output sanity.
if echo "$INSTALL_OUT" | grep -q 'self-test: 5/5 passed'; then
  _pass "self-test reports 5/5 passed"
else
  _fail "self-test did not report 5/5 passed"
  printf '  saw: %s\n' "${INSTALL_OUT:0:800}" 1>&2
fi

# Idempotency: re-run with the SAME sandbox HOME — should still exit 0 and
# leave the same file count.
set +e
