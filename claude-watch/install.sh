...
# Installs the /claude-watch skill — drop any video URL (single video, full
...
# `claude-watch-skill/` directory on main, with a local fallback when running
# from a clone.
# =============================================================================
...
# -----------------------------------------------------------------------------
# File list — keep in sync with claude-watch-skill/ contents.
# Paths are relative to claude-watch-skill/.
# -----------------------------------------------------------------------------
CW_FILES=(
    "SKILL.md"
    "scripts/__init__.py"
...
# -----------------------------------------------------------------------------
# Install the /claude-watch skill.
# Tries curl from main first; falls back to a local copy if reachable.
# -----------------------------------------------------------------------------
install_claude_watch_skill() {
    local SKILL_DIR="$HOME/.claude/skills/claude-watch"
    local BASE_URL="https://raw.githubusercontent.com/lorecraft-io/creativity-maxxing/main/claude-watch-skill"
    ...
    local LOCAL_SRC="$(dirname "$SCRIPT_DIR")/claude-watch-skill"
    ...
    info "Installing /claude-watch skill..."
    ...
    if [ "$ok" -eq 1 ]; then
        success "/claude-watch skill installed at $SKILL_DIR"
    else
        soft_fail "Could not install /claude-watch skill — download and local fallback both failed"
    fi
}
...
# states at first /claude-watch invocation via the SessionStart hook.
# -----------------------------------------------------------------------------
run_self_test() {
    local SKILL_DIR="$HOME/.claude/skills/claude-watch"
    local pass=0 fail=0
    ...
    if [ -f "$SKILL_DIR/SKILL.md" ]; then
        success "TEST: claude-watch SKILL.md installed"
        pass=$((pass + 1))
    else
        soft_fail "TEST: claude-watch SKILL.md missing"
        fail=$((fail + 1))
    fi

    if [ -f "$SKILL_DIR/scripts/watch.py" ] && [ -f "$SKILL_DIR/scripts/setup.py" ]; then
        success "TEST: claude-watch scripts installed (watch.py + setup.py present)"
        pass=$((pass + 1))
    else
        soft_fail "TEST: claude-watch scripts missing"
        fail=$((fail + 1))
    fi
    ...
    if [ -f "$SKILL_DIR/commands/claude-watch.md" ]; then
        success "TEST: /claude-watch slash-command entry point installed"
        pass=$((pass + 1))
    else
        soft_fail "TEST: commands/claude-watch.md missing"
        fail=$((fail + 1))
    fi
...
print_summary() {
    ...
    echo "    /claude-watch — frame-aware video → structured study notes"
    echo "                    (15 files: SKILL.md + 11 scripts + hook + command + plugin glue)"
    echo ""
    echo "  Use it: paste any URL — single video, channel, or playlist:"
    echo "    /claude-watch https://youtu.be/<id>"
    echo "    /claude-watch https://www.youtube.com/@<channel>/videos --limit 10"
    echo "    /claude-watch ~/path/to/local.mp4"
    echo ""
...
main() {
    ...
    echo -e "${BLUE}  /claude-watch — frame-aware video → structured study notes${NC}"
    echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
