...
- **Profile (binding)** — a client with a signed marketing agreement. Loads that client's profile and enforces it as hard rules. **LAVA is binding** → `references/profiles/lava.md`. When the copy is for Lava / LAVA-NET / Lava Foundation, load that profile and apply it verbatim — it overrides generic guidance where stricter.
...
- `references/profiles/lava.md` — **binding** LAVA contract gate: exact banned lexicon, closed vocab (Consumers/Developers/Validators/Providers), imagery, source examples, escalation. Backed by `01-Projects/LAVA-NET/compliance/`.
