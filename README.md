...
- **`/claude-watch`** — drop any video URL (single video, full YouTube channel, or playlist) and get back timestamped, frame-aware study notes. Splits video into scene-change frames + a transcript (captions free, local whisper.cpp fallback — runs key-free off the media-module stack), then Claude reads every frame as an image and writes a section-per-scene `notes.md` to `~/claude-watch/library/`. Channel mode rolls up an `index.md` with cross-channel synthesis (recurring hooks, formats, cloneable moves).
