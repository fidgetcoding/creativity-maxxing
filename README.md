...
design, video, audio, transcription, copywriting, frame-aware video study in one install.**
...
- **`/humanizer`** — a second pass that scrubs the 33 tells of AI writing catalogued by Wikipedia's WikiProject AI Cleanup: em dashes, rule-of-three, inline-header lists, "stands as a testament to," tailing negations, manufactured punchlines. Run it after `/copywriting`, never alongside it.
- **`/watch`** — drop any video URL (single video, full YouTube channel, or playlist) and get back timestamped, frame-aware study notes. Splits video into scene-change frames + a transcript (captions free, local whisper.cpp fallback — runs key-free off the media-module stack), then Claude reads every frame as an image and writes a section-per-scene `notes.md` to `~/watch/library/`. Channel mode rolls up an `index.md` with cross-channel synthesis (recurring hooks, formats, cloneable moves).
...
| `/video-shotcraft` | Claude skill (third-party, commit-pinned) | Shot design for **product** video — framing, camera move, lighting and pacing, card by card. From `Vincentwei1021/video-shotcraft` (Apache-2.0), pinned at `0022ec4`. Orthogonal to `/watch`: this is for the thing on the table, not the person at the desk. **The ~104 shot cards are written in Chinese** — the scaffolding is English and Claude reads them fine, but don't open the reference files expecting English prose. |
| AI film pipeline | Claude skills (5, third-party, commit-pinned) | A narrative-video chain: `/ai-film-director` (end-to-end director pass), `/story-bible-builder` (canon + character + world consistency), `/cinema-worldbuilder-pro-30` (world design), `/banana-pro-director-3.0` (shot + prompt direction), `/video-qa` (QA on generated footage). From `A0339x/ai-film-pipeline` (MIT), pinned at `5314221`. See the provenance note below. |
...
| `/humanizer` skill | Claude skill (third-party, commit-pinned) | AI-tell scrubber from `blader/humanizer`, pinned at `523374d`. 33 patterns drawn from Wikipedia's "Signs of AI writing": inflated significance, promotional language, superficial `-ing` analyses, vague attribution, em dashes, rule of three, AI vocabulary, copula avoidance, negative parallelism, sycophancy, filler, hedging. Installed by the copywriting module; bump `HUMANIZER_COMMIT` in `copywriting/install.sh` to update. |
| `/watch` skill | Claude skill | Frame-aware video → structured study notes. Accepts a single video URL, a full YouTube channel URL, or a playlist URL. yt-dlp pulls the video, ffmpeg extracts scene-change frames, whisper.cpp (local, key-free — reuses the media module's `~/.whisper/ggml-base.en.bin`) transcribes when captions are missing; Claude reads every frame as an image and writes `~/watch/library/<slug>/notes.md`. Channel mode also writes `index.md` with cross-channel synthesis (recurring hooks, formats, cloneable moves). Groq / OpenAI Whisper still available as cloud fallbacks. 15 files: SKILL.md + 11 Python scripts + a SessionStart hook + slash-command entry point. |

**Provenance note on the AI film pipeline.** This is the thinnest dependency chain in the repo and it's worth saying so plainly. The upstream is a small personal collection — single-digit stars, no forks, first published 2026-05-14. Its `LICENSE` is MIT and does grant redistribution, but GitHub reports `NOASSERTION` because of a custom copyright preamble, and two of the five skills (`banana-pro-director-3.0`, `cinema-worldbuilder-pro-30`) originate with Joey of *CTRL: Hunters*, included on the strength of a spoken "take them, use them, modify them" grant made in a YouTube video rather than a written license. The skills themselves are good. The paperwork is not airtight. That is exactly why it's commit-pinned: if any of it is pulled or rewritten, your install stays reproducible and the failure is loud instead of silent. If that trade isn't one you want, skip the module.

Two of these five install under a **different name than their folder** — upstream's `name:` frontmatter carries a version suffix (`banana-pro-director/` declares `name: banana-pro-director-3.0`). The frontmatter name is what you type as the slash command, so `/banana-pro-director-3.0` and `/cinema-worldbuilder-pro-30` are correct, if clunky.
...
- **`/humanizer`** — the finishing pass, and the one I reach for on anything a client reads. `/copywriting` writes; `/humanizer` audits what got written. It comes from `blader/humanizer` and codifies Wikipedia's "Signs of AI writing" into 33 checkable patterns, so instead of vibes it gives me named defects: inline-header bullet lists, tailing negations ("no guessing"), aphorism formulas ("X is the language of Y"), staccato drama, "the real question is," curly quotes, and a hard ban on em dashes. Order matters. Running both skills on one draft at the same time produces mush, because they optimize for different things — one for persuasion, one for un-detectability. Draft with `/copywriting`, then hand the result to `/humanizer`.
